...
LABEL = 'final3651-native6b3-json-performance-r2'
...
def run_version(args, name, version, target, deployment, out, weights, historical, harness):
    ...
    # Supported by both pinned hosts. Keep every frozen-file hash intact while
    # placing per-job mutable logs outside the immutable deployment.
    file_logs = (out / f'{name}-{version}-file-logs').resolve()
    env['TENSORSHARP_LOG_DIR'] = str(file_logs)
    report['logging_environment'] = {'TENSORSHARP_LOG_DIR': str(file_logs)}
