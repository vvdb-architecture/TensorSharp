#!/usr/bin/env python3
"""Offline serial-only phase correlation for the reviewed four-job diagnostic."""
import hashlib
import json
...
RAW = Path('/tmp/deepseek41-reference/qwen35-managed-native-cross-phase-r1')
OUT = Path('/tmp/deepseek41-reference/qwen35-cross-phase-analysis')
...
assert manifest['run_complete'] and manifest['all_cases_passed'] and len(manifest['runs']) == 4
...
result = {'diagnostic_only': True, 'performance_qualified': False, 'manifest_source': source(RAW / 'run.json'),
          'analyzer_source': source(Path(__file__)), 'all_measured_cases_passed': 60,
          'solo_correlated_cases': 12, 'runs': runs, 'managed_effects_at_fixed_native': effects,
          'finding': 'Final managed builds retain a higher solo TTFT median with either native library in this one fixed-order crossing. The roughly27–29ms native verify median does not carry the earlier15ms TTFT increase; native6b3 is not a consistent added-cost component in these observations.',
          'limits': 'Only3 solo requests/job; fixed order and phase instrumentation are diagnostic. Retained native bind outliers occur in baseline/base and both final-managed jobs. The difference between integer server TTFT and native phase total is an unresolved residual, not a separately instrumented grammar/scheduler/GC measurement. Asynchronous timing boundaries can overlap. This evidence does not prove a specific managed source change or establish a regression fix. Concurrent phase lines remain aggregate.',
          'loaded_library_observer_source': source(Path('/tmp/deepseek41-reference/qwen35-cross-loaded-libraries.json'))}
