...
for f in sorted(a.source.glob(a.profile+'-*.json')):
 try: data=json.loads(f.read_text())
  ...
  raw_name=f.name+'.invalid-json.txt'; (a.output/raw_name).write_bytes(f.read_bytes())
  print(json.dumps({'invalid_json_source':str(f),'raw_artifact':raw_name,'error':str(error)})); continue
