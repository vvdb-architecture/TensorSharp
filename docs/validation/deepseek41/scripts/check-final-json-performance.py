...
with tempfile.TemporaryDirectory() as directory:
    ...
    original = {str(p): p.read_bytes() for host in hosts.values() for p in host.iterdir()}
    ...
    for mode in ('all-matched', 'changed-valid-response', 'telemetry-finalization-failure'):
        ...
        assert {str(p): p.read_bytes() for host in hosts.values() for p in host.iterdir()} == original
...
output = Path('/tmp/deepseek41-reference/final-json-performance-checks.json')
