...
在网络存储上，首次访问稀疏的 Engram 行可能主导 prefill 与 decode 延迟。
`TS_DSV41_ENGRAM_WARM=1` 可在模型加载时读入 Engram 表页。它只读这些表，不创建私有
副本或 pinned 分配，页面仍可被操作系统回收。除非映射的主机权重再加 8 GiB 能装进探测到
的主机/cgroup 内存额度，否则预热会被跳过并给出诊断。冷启动与热态推理要分开测量。
...
- `ggml_cuda` 是 V4.1 唯一的服务后端。`ggml_cpu` 用标量 CPU 实现加载同一套原生计算图，
  作为正确性与可移植性通道，没有实测吞吐；见
  [在 ggml CPU 后端上运行](#在-ggml-cpu-后端上运行)。其余所有后端——包括实现了 V4 的
  `cpu` 与 `cuda`——都会在读取权重之前失败，而不会把 V4.1 的权重塞进 V4 的计算图。
