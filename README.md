...
- **📱 TensorAgent — the whole thing on an iPhone.** An iOS/iPadOS app with the Web UI chat's full functionality, running entirely on the device: the engine linked statically as an `.xcframework` with ggml-metal, and `TensorSharp.Server/wwwroot/index.html` served **byte for byte** to a WKWebView by an in-process loopback server, so the page is the desktop's page rather than a port of it. The API under it is the same `WebUiChatService`, which is why the chat pipeline was pulled out of ASP.NET Core in the first place — iOS has no runtime pack for it. A built-in catalog of eight phone-sized models (Gemma 4 and Qwen 3.5/3.6/3.8, dense **and** MoE, plus Qwen-Image-Edit) downloads with resume and SHA-256 verification, filtered by the device's own memory. Chats are saved host-side and resume across app kills, because the page keeps its history only in memory. Photos, camera, video, files and on-device dictation feed the same `/api/upload` the paperclip uses; HEIC decodes through ImageIO, which the managed decoder cannot read at all. **`Process.Start` is unsupported on iOS**, so the agent host's shell tool is backed by an in-process POSIX shell (pipelines, heredocs, globs, functions, and the coreutils a coding model reaches for, awk included), an embedded CPython 3.13 whose sandbox is an audit hook, and JavaScriptCore — with confinement moved from the kernel into the runtimes and reported honestly, including the one thing it cannot do, which is preempt a builtin already inside a long call. Twelve skills are bundled, chosen by a checker that resolves every script's imports against the staged interpreter rather than by hope. Measured on Gemma 4 E4B Q8_0: a four-turn conversation reuses **99.2–99.3%** of each prompt's KV cache, and a new chat correctly drops to zero. → [TensorAgent](TensorAgent/README.md)
