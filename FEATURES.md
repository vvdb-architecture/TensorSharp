...
- **Multi-architecture support** -- DeepSeek V4 Flash, DeepSeek V4.1 Flash (`deepseek41`, served on `ggml_cuda`; also runs on `cpu`, `cuda` and `ggml_cpu`), GLM 5.x (GLM-5.2 `glm-dsa` and GLM-5.3-Flash `glm5next`), Gemma 4, DiffusionGemma, Qwen 3.5/3.6-family, Qwen 3.8 Flash Next (`qwen4exp`), GPT OSS, Nemotron-H, Mistral 3, Hunyuan Dense (`hunyuan-dense`), Muse-Glimmer, Qwen-Image-Edit (image editing), MiniMax-H3 (video with native 32 kHz stereo audio), and Wan 2.1/2.2 (video only)
